...
object BinaryTrees {
     ...
     *
     * A trait is similar to abstract class but without a constructor.  In this
     * case the trait has no members.  Trait are commonly used in Scala to provide
     * reusable code that can be "mixed in" to other classes.  In this aspect they
     * are similar to Java interfaces but can contain values and implementations
     * rather than just declarations.
     */
    trait Tree
